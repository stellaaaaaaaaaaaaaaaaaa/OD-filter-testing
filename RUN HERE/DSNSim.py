...
def generate_DSNSim_from_truth(truth_data, tk):
    ...
    def MCF_to_ECI(coordinates, time):
        ...
        Moon_State_Data = np.zeros((len(time), 6))  
        
        for i in range(len(coordinates)):
            
            et = time[i]
            Moon_State = spice.spkezr('301', et, 'J2000', 'NONE', '399')[0]
            Moon_State_Data[i] = Moon_State
            
            MoonPositionData = Moon_State_Data[i, :3]
            MoonVelocityData = Moon_State_Data[i, 3:6]
            
            converted_coord[i, 0:3] = coordinates[i, 0:3] + MoonPositionData
            converted_coord[i, 3:6] = coordinates[i, 3:6] + MoonVelocityData
    ...
    def ECI_to_MCI(coordinates, time):
        ...
        ECI_to_MCI_coord = np.zeros_like(coordinates) #initialise empty array
        ...
        for i in range(len(coordinates)):
            ...
            ECI_to_MCI_position = coordinates[i, 0:3] - MoonPositionData
            ECI_to_MCI_velocity = coordinates[i, 3:6] - MoonVelocityData
            ...
            ECI_to_MCI_coord[i, 0:3] = ECI_to_MCI_position
            ECI_to_MCI_coord[i, 3:6] = ECI_to_MCI_velocity
        
        return ECI_to_MCI_coord
    
    DSN_Sim_Data = ECI_to_MCI(DSN_sim_coord, tk)
