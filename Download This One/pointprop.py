...
def point_propagation(XREF0, tkminus1, tk):
    ...
    #uses HALO propagation but only for one time to the next
    
    spice.kclear()
    spice.furnsh('metakernel.tm')
    
    orb = {}
    orb = LoadModel("Fast", orb) #can also change to 'fast' vs. "Ref" -- this may actually help simulate the real differences between propagator and real life
...
def STM(XREFk, dt):
    ...
    max_time = 5.0  #maximum time for STM calculation so if it breaks its not forever stuck
...
def STM_from_states(X0, Xf, t0, tf, orb=None):
    ...
    #STM using HALO prop and more complex harmonics as CR3BP no longer is adequate for HALO
    #basically incorporates the harmonics into the STM formation
    if orb is None:
        orb = {}
    ...
    #come from Yang support STM file
    ...
    #compute STM using matrix exponential like for original CR3BP
    STM = expm(A * dt)
